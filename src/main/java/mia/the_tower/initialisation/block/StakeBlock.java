...
public class StakeBlock extends Block implements Fertilizable {
    ...
    public static final int field_31000 = 27;
    ...
    @Override
    public MapCodec<StakeBlock> getCodec() {
        return CODEC;
    }

    public StakeBlock(AbstractBlock.Settings settings) {
        super(settings);
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        if (!fluidState.isEmpty()) {
            ...
            if (blockState.isIn(BlockTags.BAMBOO_PLANTABLE_ON)) {
                if (blockState.isOf(stake_init.STAKE_BUD)) {
                    return this.getDefaultState().with(AGE, 0);
                } else if (blockState.isOf(stake_init.STAKE)) {
                    ...
                    return blockState2.isOf(stake_init.STAKE) ? this.getDefaultState().with(AGE, (Integer)blockState2.get(AGE)) : stake_init.STAKE_BUD.getDefaultState();
                }
            } else {
                return null;
            }
    ...
    protected void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if ((Integer)state.get(STAGE) == 0) {
            if (random.nextInt(20) == 0 && world.isAir(pos.up()) && world.getBaseLightLevel(pos.up(), 0) >= 1) {
    ...
    protected boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
        return world.getBlockState(pos.down()).isIn(BlockTags.BAMBOO_PLANTABLE_ON)
                || world.getBlockState(pos.down()).isOf(stake_init.STAKE)
                || world.getBlockState(pos.down()).isOf(stake_init.STAKE_BUD);
    }
    ...
    protected BlockState getStateForNeighborUpdate(
        ...
        return direction == Direction.UP && neighborState.isOf(stake_init.STAKE) && neighborState.get(AGE) > state.get(AGE)
                ? state.cycle(AGE)
    ...
    public boolean isFertilizable(WorldView world, BlockPos pos, BlockState state) {
        ...
        return i + j + 1 < 27 && (Integer)world.getBlockState(pos.up(i)).get(STAGE) != 1;
    }
    ...
    public void grow(ServerWorld world, Random random, BlockPos pos, BlockState state) {
        ...
        for (int m = 0; m < l; m++) {
            ...
            if (k >= 27 || (Integer)blockState.get(STAGE) == 1 || !world.isAir(blockPos.up())) {
                return;
            }
    ...
    protected void updateLeaves(BlockState state, World world, BlockPos pos, Random random, int height) {
        ...
        if (height >= 1) {
            if (!blockState.isOf(stake_init.STAKE) || blockState.get(LEAVES) == BambooLeaves.NONE) {
                bambooLeaves = BambooLeaves.SMALL;
            } else if (blockState.isOf(stake_init.STAKE) && blockState.get(LEAVES) != BambooLeaves.NONE) {
                bambooLeaves = BambooLeaves.LARGE;
                if (blockState2.isOf(stake_init.STAKE)) {
        ...
        int i = state.get(AGE) != 1 && !blockState2.isOf(stake_init.STAKE) ? 0 : 1;
        int j = (height < 22 || !(random.nextFloat() < 0.25F)) && height != 26 ? 0 : 1;
    ...
    protected int countBambooAbove(BlockView world, BlockPos pos) {
        ...
        while (i < 27 && world.getBlockState(pos.up(i + 1)).isOf(stake_init.STAKE)) {
            i++;
        }
    ...
    protected int countBambooBelow(BlockView world, BlockPos pos) {
        ...
        while (i < 27 && world.getBlockState(pos.down(i + 1)).isOf(stake_init.STAKE)) {
            i++;
        }
